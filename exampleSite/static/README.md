...
The site uses `favicon.svg` as the primary favicon (modern browsers). For older
browser support (IE), you should generate a `favicon.ico` file.
...
- `<link rel="icon" href="{{ "favicon.svg" | relURL }}" type="image/svg+xml">`
  (modern browsers)
- `<link rel="icon" href="{{ "favicon.ico" | relURL }}" sizes="any">` (fallback
  for older browsers)
